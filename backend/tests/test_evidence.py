...
async def test_evidence_sets_verified_when_text_available():
    ...
    mock_co_bill.full_text = None  # not yet fetched — fall through to cache
...
async def test_evidence_uses_full_text_from_db_for_co_bill():
    """If full_text is set on the CO bill, no cache or API call needed."""
    from worker.tasks.evidence import _extract_evidence_for_match
    ...
    mock_co_bill = MagicMock()
    mock_co_bill.legiscan_id = 1
    mock_co_bill.full_text = co_text  # already fetched — should skip cache
    mock_corpus_bill = MagicMock()
    mock_corpus_bill.legiscan_id = 2
    ...
    mock_cache = AsyncMock()
    mock_cache.get_bill_text.return_value = src_text  # called for corpus bill only
    mock_client = AsyncMock()

    await _extract_evidence_for_match(mock_session, mock_match, mock_cache, mock_client)

    assert mock_match.snippet_status == "verified"
    # Cache called once (corpus bill only), not twice
    assert mock_cache.get_bill_text.call_count == 1


async def test_evidence_falls_back_to_api_on_cache_miss():
    from worker.tasks.evidence import _extract_evidence_for_match
    ...
    mock_co_bill = MagicMock()
    mock_co_bill.legiscan_id = 1
    mock_co_bill.full_text = None
    mock_co_bill.text_doc_id = 111
    mock_corpus_bill = MagicMock()
    mock_corpus_bill.legiscan_id = 2
    mock_corpus_bill.text_doc_id = 222
    ...
    mock_client = AsyncMock()
    mock_client.get_bill_text_by_doc_id.side_effect = [co_text, src_text]

    await _extract_evidence_for_match(mock_session, mock_match, mock_cache, mock_client)

    assert mock_match.snippet_status == "verified"
    assert mock_cache.set_bill_text.call_count == 2
    mock_client.get_bill_text_by_doc_id.assert_any_call(111)
    mock_client.get_bill_text_by_doc_id.assert_any_call(222)
...
async def test_evidence_sets_ghost_when_text_unavailable():
    ...
    mock_cache.get_bill_text.return_value = None
    mock_client = AsyncMock()
    mock_client.get_bill_text_by_doc_id.return_value = None
