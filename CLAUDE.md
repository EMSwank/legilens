...
| Cron Un-gate (WS2, NY deferred) | Un-gate the nightly fetch from CO-only to CO + tier-1 comparison states **excluding NY**, via `max_priority_tier=1` on `next_queued_bills` + demoting NY from tier 1 → tier 2 in `queue._STATE_PRIORITY` (`_TOP5_STATES`→`_TIER1_STATES`). Lets the corpus accrue cross-state MinHash signatures so real cross-state copycat alerts can surface (corpus load is `is_corpus_only`-scoped, not state-scoped → no match-phase change). Coverage `SCOPE` drops NY in lockstep, else its ~150k never-fetched bills pin the matchable-% thermometer ~14%. NY (~150k bills/~6.5mo alone) deferred until the national tail is funded + Neon spend cap raised; re-promote = add NY back to `_TIER1_STATES` **and** `SCOPE`. Burst path stays CO-only. Reversible, zero migrations. | 🟡 PR open — **merge gated on Neon spend cap** |
...
- `backend/app/services/coverage.py` — pure coverage helpers (aggregate / derive status / scoped matchable %); `SCOPE` = CO + 4 comparison states (CA/IL/TX/FL; NY deferred to tier 2 in WS2), kept in sync with `queue._STATE_PRIORITY`
- `backend/tests/` — pytest-asyncio suite (185 tests), all API tests using `dependency_overrides` (not patch); worker tests use `unittest.mock.patch`
...
- **WS3 Coverage Tracker (PR #62):** the nightly `fetch_and_match` computes a per-state `{fetchable, with_sig}` snapshot and upserts it as JSON in `worker_state` (key `coverage_snapshot`). Counting is **pure Python** (`app/services/coverage.py`), not a SQL `GROUP BY`, because the test suite is mock-only and the models are Postgres-locked (`postgresql.UUID` / `ARRAY` / `JSONB`) — so the hazard logic must be unit-testable with no DB. The read emits one boolean-per-bill row via `EXISTS` (never a join → duplicate signature rows can't fan-out double-count `fetchable`); `with_sig` is AND-gated on `text_doc_id IS NOT NULL` so a signature on a doc-less bill can't push the matchable ratio above 1. `as_of` is read from `worker_state.updated_at`, set **explicitly** in the upsert `set_=` (the model's `onupdate=func.now()` does not fire on the `on_conflict_do_update` path). The headline matchable-% denominator is scoped to `SCOPE` (CO + 4 comparison states — CA/IL/TX/FL; **NY deferred to tier 2 in WS2** so its ~150k unfetched bills don't pin the thermometer ~14%), which mirrors `queue._STATE_PRIORITY`. The snapshot call is wrapped in try/except so it can never break the cron, and runs even on 0-fetch nights. Zero migrations.
- **Corpus comparison ingest pass (TX=0 root-cause fix):** `run_full_pipeline` runs a bounded per-state ingest of the cross-state comparison states (`_TIER1_INGEST_STATES = CA/IL/TX/FL`, via `_ingest_scope_states`) **between** pass 1 (CO) and pass 2 (full march), using the same `getDatasetList(state=)` server-side filter pass 1 uses for CO. **Root cause it fixes:** `getDatasetList` returns ~995 datasets ordered by `state_id`; pass 2 iterates from index 0 every run while dataset-dedup re-downloads weekly-changed early-state datasets, so a full sweep never completes and the `state_id>=34` tail is never reached. TX is `state_id` 43 → prod had `bills.TX = 0` (CA/IL/FL are `state_id<=33`, so they land in pass 2's early window). The scope pass makes the comparison corpus independent of pass 2 finishing: CA/IL/FL dedup-skip without a download, only TX's ~23 datasets transfer (~200 MB). NY is excluded — `state_id` 32, already fully ingested (~185k bills); re-pulling it every run is pure cost. Best-effort per state (one state failing is logged + skipped; the nightly rerun retries it). Zero migrations. **Caveat:** this lands TX *metadata* only — TX enters cross-state matching solely once its text is fetched + signatures computed, and steady-state text fetch is CO-scoped on main (the WS2 tier-1 un-gate is a separate, quota-gated decision).
...
- `/coverage` renders the accessible per-state table as the source of truth (status shown as text, not color alone); the inline status dots are `aria-hidden` decoration. The headline matchable % is scoped to CO + 4 comparison states (NY deferred) and labeled as such; the `pending` cold-start state and a `null` matchable % are handled distinctly from each other.
